...
class MultiCutStixelData(Dataset):
    ...
    def _preparation_of_target_label(self, y_target: pandas.DataFrame, grid_step: int = config['grid_step']) -> torch.tensor:
        ...
        y_target['yT'] = (y_target['yT'] / self.img_size['height'])
        y_target['yB'] = (y_target['yB'] / self.img_size['height'])
        maximum_dist = config['max_dist']    #y_target['depth'].max()
        y_target.loc[y_target['depth'] > maximum_dist, 'depth'] = maximum_dist
        y_target['depth'] = (y_target['depth'] / maximum_dist)
        mtx_height, mtx_width = int(config['num_obj_per_col']), int(self.img_size['width'] / grid_step)
        stixel_mtx = np.zeros((3, mtx_height, mtx_width))
        # 4 dim: yT, yB, dist, probab.
        for index, stixel in y_target.iterrows():
            if stixel['class'] == 1:
                col = stixel['x']
                for i in range(mtx_height):
                    if stixel_mtx[2, i, col] == 0:
                        stixel_mtx[0, i, col] = stixel['yT']
