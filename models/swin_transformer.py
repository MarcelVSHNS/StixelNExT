...
with open('models/config_swin.yaml') as yamlfile:
...
class SwinEmbedding(nn.Module):
    ...
    """
    def __init__(self, patch_size=config['patch_size'], c=96):
        super().__init__()
...
class ShiftedWindowMSA(nn.Module):
    def __init__(self, embed_dim, num_heads, window_size=config['window_size'], mask=False, device="cpu"):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.mask:
            row_mask = torch.zeros((self.window_size**2, self.window_size**2)).to(self.device)
            row_mask[-self.window_size * (self.window_size//2):, 0:-self.window_size * (self.window_size//2)] = float('-inf')
            row_mask[0:-self.window_size * (self.window_size//2), -self.window_size * (self.window_size//2):] = float('-inf')
            column_mask = rearrange(row_mask, '(r w1) (c w2) -> (w1 r) (w2 c)', w1=self.window_size, w2=self.window_size).to(self.device)
...
class SwinEncoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, window_size, mask, device="cpu"):
        super().__init__()
        self.layer_norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(0.1)
        self.WMSA = ShiftedWindowMSA(embed_dim=embed_dim, num_heads=num_heads, window_size=window_size, mask=mask, device=device)
...
class AlternatingEncoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, window_size=config['window_size'], device="cpu"):
        super().__init__()
        self.WSA = SwinEncoderBlock(embed_dim=embed_dim, num_heads=num_heads, window_size=window_size, mask=False, device=device)
        self.SWSA = SwinEncoderBlock(embed_dim=embed_dim, num_heads=num_heads, window_size=window_size, mask=True, device=device)
...
class SwinTransformer(nn.Module):
    def __init__(self, device="cpu"):
        ...
        self.Stage1 = AlternatingEncoderBlock(96, 3, device=device)
        self.Stage2 = AlternatingEncoderBlock(192, 6, device=device)
        self.Stage3_1 = AlternatingEncoderBlock(384, 12, device=device)
        self.Stage3_2 = AlternatingEncoderBlock(384, 12, device=device)
        self.Stage3_3 = AlternatingEncoderBlock(384, 12, device=device)
        self.Stage4 = AlternatingEncoderBlock(768, 24, device=device)
